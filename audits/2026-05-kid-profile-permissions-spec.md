...
- **Chore completion (Batch 3 Half B + Batch 4)**: all chore completions now go through RPCs (Half B established the pattern).
  ...
  - **Admin approve / reject (shipped Half B)**: the admin verify flow calls `approve_chore` RPC. Reject sets `status='rejected'` (kid can Re-do via Batch 4b's redo_chore RPC; admin reject-with-reason dialog also lands in 4b).
  - **Kid completion (Batch 4)**: 3-button choice dialog (Take Photo / Skip Photo / Cancel). If "Take Photo": camera opens → upload to `chore-photos` bucket → row inserts into `chore_verification_photos` → status transitions to `'pending_verification'`. If "Skip Photo": no photo upload, no row inserted; status still transitions to `'pending_verification'`. Cancel = no-op. The same RPC `submit_kid_chore_with_photo` handles both paths (migration 0019 made `p_storage_path` nullable; the photo INSERT is conditional on `v_has_photo`). Adults' no-photo path (`complete_chore_self`) unchanged. (REVISED 2026-05-24 per Q-A reversal; commit ed626bb.)
...
| **4a** ✅ | Chore submission flow with **optional** photo: kid 3-button choice (Take Photo / Skip Photo / Cancel), conditional storage upload, RPC accepts null path (migration 0019). Replaces the Half-B kid TODO paths in `chore_dashboard_screen.dart:_completeChore` and `chore_detail_screen.dart:_quickUpdateStatus` with the new flow. Adults' `complete_chore_self` path unchanged. (Per Q-A reversal 2026-05-24 — was originally "required for kids".) | Medium | Batches 1+2+3 Half A+B | Shipped: commit `ed626bb` on `feat/kid-perms-batch-4-kid-photo-flow-2026-05-24` |
| **4b** ✅ | Close the chore-photo loop. (1) Re-do RPC — `redo_chore(p_chore_id, p_member_id)` flips a rejected chore back to `'assigned'` and clears rejection metadata. (2) Admin reject-with-reason dialog — multiline TextField (maxLength 500); `approve_chore`'s existing `p_reason` is now populated from UI. (3) Photo viewer widget — `apps/mobile/lib/widgets/chore_photo_viewer.dart` with signed-URL thumbnail + pinch-to-zoom full-screen modal + empty state for 4a Skip submissions. (4) Dashboard `'rejected'` UI mapping — coral badge + truncated reason snippet + Re-do FilledButton in `_ChoreCard`; `_VerificationCard` now renders the photo thumbnail. (5) Admin delete-photo — `delete_chore_photo(p_photo_id) RETURNS text` validates admin, deletes the row, returns the storage_path; client calls `storage.from('chore-photos').remove([path])`. Migration 0020 adds both new RPCs. | Medium-High | Batch 4a | Shipped: commit `cc3da3d` on same branch |
